...
export const RegexNode = memo(({ data, selected }: NodeProps) => {
  ...
  return (
    <div className={`min-w-[240px] rounded-lg border-2 bg-card shadow-sm ${selected ? "border-emerald-500" : "border-border"}`}>
      ...
      <div className="flex items-center gap-2 rounded-t-md bg-emerald-700 px-3 py-2">
        ...
        <span className="text-xs font-semibold text-white">{t("canvas.nodes.regex")}</span>
      </div>
      <div className="space-y-1 p-3 text-xs text-muted-foreground">
        ...
        <p className="truncate">{t("canvas.fields.outputMode")}: <span className="font-mono text-foreground">{config.outputMode ?? "fanout"}</span></p>
      </div>
