...
export const UnifiedAccordionBrick: React.FC<AccordionBrickProps> = ({ 
  id, title, body, isExpanded, onUpdate, readonly, documents, boards, activeBricks 
}) => {
    ...
    <div className="w-full border-b border-border/40 last:border-0 overflow-hidden transition-all duration-300">
      ...
      >
        ...
        {readonly ? (
          ...
          <input 
            className="flex-1 bg-transparent border-none outline-none focus:ring-0 p-0 text-sm font-semibold placeholder:text-muted-foreground/30 leading-none"
            value={title}
