...
export function UnifiedFormBrick({
  ...
  const handleTableSelection = (selection: TableBrickPickerSelection) => {
    setShowTablePicker(false);
    
    let newFields = legacyFields;
    if (selection.brickContent?.columns && Array.isArray(selection.brickContent.columns)) {
      newFields = generateFormFieldsFromColumns(selection.brickContent.columns);
    }
    
    const dbUrl = `database:\\\\${selection.source}/${selection.documentId || selection.meshId || "local"}?[${selection.brickId}]`;

    updateContent({
      tableConnectorId: selection.brickId,
