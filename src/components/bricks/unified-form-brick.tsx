...
export function UnifiedFormBrick({
  ...
  const handleTableSelection = async (selection: TableBrickPickerSelection) => {
    ...
    try {
      ...
      const nextChildrenByContainer = { ...childrenByContainer };
      if (pseudoFieldEntries.length > 0) {
        for (const page of safePages) {
          if (nextChildrenByContainer[page.id]) {
            nextChildrenByContainer[page.id] = nextChildrenByContainer[page.id].filter(
              (id) => !pseudoFieldEntries.some((e) => e.brickId === id)
            );
          }
        }
      }
      ...
      updateContent({
        ...
        fields: newFields,
        childrenByContainer: pseudoFieldEntries.length > 0 ? nextChildrenByContainer : childrenByContainer,
